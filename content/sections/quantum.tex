...
%\label{ssub:ward_identities}
%
%The symmetries of the action $\Sigma$ are
%
%The usual gauge fixing equation
%\begin{equation}
%  \label{eq:gfeq}
...
%\end{equation}
%The Faddeev-Popov anti-ghost equation
%\begin{equation}
...
%\begin{align}
%  \label{eq:SToperator}
%  \mathcal{S} & \equiv \varint\mathrm{Tr}\left[\left(\psi-\frac{\delta\phantom{\Omega}}{\delta\Omega}\right)\frac{\delta\phantom{A}}{\delta A}+\left(\phi-\frac{\delta\phantom{L}}{\delta L}\right)\frac{\delta\phantom{c}}{\delta c}-\frac{\delta\phantom{\tau}}{\delta\tau}\frac{\delta\phantom{\psi}}{\delta\psi}-\frac{\delta\phantom{E}}{\delta E}\frac{\delta\phantom{\phi}}{\delta\phi}+X\frac{\delta\phantom{Y}}{\delta Y}+b\frac{\delta\phantom{\bar{c}}}{\delta\bar{c}}+B\frac{\delta\phantom{\bar{\chi}}}{\delta\bar{\chi}}+\bar{\eta}\frac{\delta\phantom{\bar{\phi}}}{\delta\bar{\phi}}\; +\right. \nonumber \\
%              & + \left. \Omega\frac{\delta\phantom{\tau}}{\delta\tau}+L\frac{\delta\phantom{E}}{\delta E}+K\frac{\delta\phantom{\lambda}}{\delta\lambda}+H\frac{\delta\phantom{Z}}{\delta Z}\right]\;
%\end{align}
%
%The bosonic anti-ghost equation
%\begin{equation}
...
%where
%\begin{equation}
%  \mathcal{G}_\phi \equiv \varint \left(\frac{\delta\phantom{\phi}}{\delta\phi}-\left[\bar{\phi},\frac{\delta\phantom{b}}{\delta b}\right]\right)\;,
%\end{equation}
%and
%\begin{equation}
%  \label{eq:linearbreakphi}
%  \Delta_\phi \equiv \varint \left(\left[A,\tau\right]+\left[c,E\right]+\left[\bar{\chi},\lambda\right]+\left[Y,Z\right]\right) \;
%\end{equation}
%is a breaking term. It, however, is linear in the quantum fields, namely $A$ and $c$.
%
%The first Faddeev-Popov ghost equation,
%\begin{equation}
...
%  \mathcal{G}^{\left(1\right)}_c \equiv \varint \left(\frac{\delta\phantom{c}}{\delta c}-\left[Y,\frac{\delta\phantom{X}}{\delta X}\right]-\left[\bar{c},\frac{\delta\phantom{b}}{\delta b}\right]-\left[\bar{\phi},\frac{\delta\phantom{\bar{\eta}}}{\delta\bar{\eta}}\right]-\left[\bar{\chi},\frac{\delta\phantom{B}}{\delta B}\right]-\left[\lambda,\frac{\delta\phantom{K}}{\delta K}\right]-\left[Z,\frac{\delta\phantom{H}}{\delta H}\right]\right)\;,
%\end{equation}
%and
%\begin{equation}
%  \label{eq:linearbreakc}
%  \Delta_c \equiv \varint \left(\left[A,\Omega\right]+\left[L,c\right]+\left[\tau,\psi\right]+\left[E,\phi\right]+\left[\bar{\chi},K\right]+\left[\lambda,B\right]+\left[Y,H\right]+\left[Z,X\right]\right)\;
%\end{equation}
%is again a breaking term. It is, then again, linear in the quantum fields, here $A$ and $c$ as well as $\psi$ and $\phi$.
%
%The second Faddeev-Popov ghost equation is
%\begin{equation}
...
%\begin{align}
%  \label{eq:vecsusyop}
%  \mathcal{W} & \equiv \varint \mathrm{Tr} \left[\mathcal{L}_\xi A\frac{\delta\phantom{\psi}}{\delta\psi}+\mathcal{L}_\xi c\frac{\delta\phantom{\phi}}{\delta\phi}+\mathcal{L}_\xi Y \frac{\delta\phantom{X}}{\delta X}-\mathcal{L}_\xi\left(\bar{c}+\bar{\eta}\right)\frac{\delta\phantom{b}}{\delta b}+\mathcal{L}_\xi \bar{\chi}\frac{\delta\phantom{B}}{\delta B}+\mathcal{L}_\xi\bar{\phi}\left(\frac{\delta\phantom{\bar{c}}}{\delta\bar{c}}-\frac{\delta\phantom{\bar{\eta}}}{\delta\bar{\eta}}\right)+ \right. \nonumber \\
%              & + \left. \mathcal{L}_\xi\tau\frac{\delta\phantom{\Omega}}{\delta\Omega}+\mathcal{L}_\xi E\frac{\delta\phantom{L}}{\delta L}+\mathcal{L}_\xi \lambda\frac{\delta\phantom{K}}{\delta K}\right]
%\end{align}
%
%The non-linear bosonic symmetry is
%\begin{equation}
%  \label{eq:t-symmetry}
